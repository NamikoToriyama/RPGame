...
let rec loop state dousa_list chizu_list
  ...
  else if state.place = "部屋２"
    then (print_endline "遺跡の呪いで死んでしまった。";
...
(* ゲームの開始 *)
let yamaura_main state = try
  let world = World_parser.start World_lexer.token
				 (Lexing.from_channel (open_in "yamaura.txt")) in
